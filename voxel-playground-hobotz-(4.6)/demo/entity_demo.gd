...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.keycode == KEY_F:
		...
		#entity_manager.debug_draw_flow_lines(8, 4)  # spacing defaults to 4
		#entity_manager.debug_clear_flow_lines(y_level)
...
func _toggle_flow_field_debug() -> void:
	...
	if _debug_flow_field:
		...
		if _debug_y_level >= 0:
			...
			entity_manager.debug_clear_flow_lines(_debug_y_level)
		...
		entity_manager.debug_draw_flow_lines(_debug_y_level)
...
func _set_targets_to_raycast() -> void:
	...
	if hit != Vector3(-1, -1, -1):
		...
		if _debug_flow_field and _debug_y_level >= 0:
			#entity_manager.debug_clear_flow_field(_debug_y_level)
			entity_manager.debug_clear_flow_lines(_debug_y_level)       # draw at y=10, default spacing 4
		...
		if _debug_flow_field and _debug_y_level >= 0:
			#entity_manager.debug_draw_flow_field(_debug_y_level)
			entity_manager.debug_draw_flow_lines(_debug_y_level)       # draw at y=10, default spacing 4
