...
func _try_connect():
	...
	# Force reconnect if VoxelWorld reinitialized (new buffers/RIDs)
	var current_version: int = voxel_world.get_init_version()
	if current_version != _last_init_version:
		_connected_effects = []
		_last_init_version = current_version
