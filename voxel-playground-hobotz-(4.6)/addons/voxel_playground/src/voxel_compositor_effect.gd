...
func _initialize_compute():
	...
	# Create nearest-neighbor sampler for depth buffer sampling
	var sampler_state := RDSamplerState.new()
	sampler_state.min_filter = RenderingDevice.SAMPLER_FILTER_NEAREST
	sampler_state.mag_filter = RenderingDevice.SAMPLER_FILTER_NEAREST
	_nearest_sampler = rd.sampler_create(sampler_state)
	print("[VoxelCompositorEffect] nearest sampler created, valid=", _nearest_sampler.is_valid())
...
func _get_sampler_uniform(sampler_rid: RID, texture_rid: RID, binding: int) -> RDUniform:
	var uniform := RDUniform.new()
...
func _render_callback(p_effect_callback_type, p_render_data):
	if !rd or !_voxel_pipeline.is_valid() or !_nearest_sampler.is_valid():
		if _frame_index == 0:
			print("[VoxelCompositorEffect] _render_callback: early exit, rd=", rd != null, " pipeline_valid=", _voxel_pipeline.is_valid(), " sampler_valid=", _nearest_sampler.is_valid())
		return
	...
	for view in range(view_count):
		...
		# Set 3: Godot depth buffer (sampler2D for depth compositing)
		var depth_image := render_scene_buffers.get_depth_layer(view)
		var set3 := UniformSetCacheRD.get_cache(_voxel_shader, 3, [
			_get_sampler_uniform(_nearest_sampler, depth_image, 0),
		])
		...
		rd.compute_list_bind_uniform_set(compute_list, set3, 3)
