...
layout(set = 3, binding = 0) uniform sampler2D godotDepthSampler;
...
void main() {
    ...
    // No voxel hit — preserve whatever Godot already rendered (geometry, skybox, etc.)
    ...
    // Voxel hit — compare depth against Godot's depth buffer
    vec3 hitPos = ray_origin + t * ray_dir;

    // Project voxel hit to clip space to get NDC depth (reverse-Z: near=1, far=0)
    vec4 clipPos = cameraParams.view_projection * vec4(hitPos, 1.0);
    float voxelDepth = clipPos.z / clipPos.w;

    // Sample Godot's depth buffer
    float godotDepth = texture(godotDepthSampler, screen_uv).r;

    // Reverse-Z: higher value = closer to camera
    // If Godot geometry is closer, preserve it
    if (voxelDepth < godotDepth) {
        return;
    }

    // Voxel is closer — shade it
